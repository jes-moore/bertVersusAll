...
def prepare_df_for_training(df, maxlen):
    ...
    log.debug("Encoding sentences using BERT, this will take several minutes")
    df['bert_enc'] = df['text'].apply(lambda x: preproc_text_bert(x, bc, maxlen))
    log.debug("Encoding sentences complete")
    return df
...
def explode_classes(df):
    ...
    log.debug("Classes expoded for multilabel classification")
    return df, y, mlb.classes_


def prep_training_data_keras(df, y, test_size):
    '''
    ...
    log.debug("Training and datasets ready for training")
    return X_train, X_test, y_train, y_test


def describe_training_data(y, classes):
    log.debug(f"Training Examples = {y.shape[0]}")
    log.debug(f"Num-classes = {y.shape[1]}")
    for ix, cl in enumerate(classes):
        log.debug(f"Class = {cl}, Count = {np.sum([out[ix] for out in y])}")
    return
